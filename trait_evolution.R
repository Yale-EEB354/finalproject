phy = read.tree("viburnum.fasta.treefile")
plot(phy)
...
CompareRates.multTrait<-function(phy,x,TraitCov=F,ms.err=NULL,ms.cov=NULL){
  ...
  #Cholesky   decomposition   function   for   diagonal-constrained VCV
    ...
    #    c.mat[p,p]<-exp(b[1])
    #    c.mat[1,1]<-sqrt(sum((c.mat[p,])ˆ2))
    #    if(p>2){for (i in 2:(p-1)){
    #      c.mat[i,i]<-ifelse((c.mat[1,1]ˆ2-sum((c.mat[i,])ˆ2))>0,
    #    sqrt(c.mat[1,1]ˆ2-sum((c.mat[i,])ˆ2)), 0)
    #    }}
    #    return(c.mat)
      }
...
(evol.vcv)
  ...
  #Within-speciesmeasurementerrorandtraitcovariation matrix
